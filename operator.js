...
// 1-1. 문자연결 연산자 : + (덧셈이 아닌 문자열끼리 이어서 쓰는 연산자)
    ...
    */
    ...
    /*
    // 3. 증감 연산자 : ++, -- [+1, -1]
    // 대입연산자와 함께 사용시, 위치에 따른 결과가 달라짐    :   A = B++ vs A = ++B 와 다른 결과
    let a = 10
    let b = 20
    console.log('a : ${a}, b: ${b}');
    console.log(a++);
   ...
   // 연산의 결과가 논리값(true, false)을 반환
    let bool1 = true;
    let bool2 = false;
    console.log(bool1 && bool2); // false [둘다 true여야 최종 ture : AND 연산]
    console.log(bool1 || bool2); // ture [둘중 하나만 true면 최종 ture : OR 연산]
    console.log(bool1); // true → false [true면 false로, false면 true 최종 판정 : NOT 연산]

    console.log();
   

    // 6. 삼항 조건 연산자 : 조건 ? ture일때 실행 표현식 : false일때 실행 표현식;
